...
OneSmapperFlour <- function(LoaderPATH = "fcs_Out", Bins = 250, doCoords = FALSE, doFreq = FALSE) {
    ...
    for (factor in 2:(dim(keeptable)[2])) {
        ...
        # colorRampPalette(c('black','orange','yellow','white'))(n=length(breaks)-1) my_palette <-
        # colorRampPalette(c('black','blue','green','yellow'))(n=length(breaks)-1) my_palette <-
        # colorRampPalette(c('blue','green','yellow'))(n=length(breaks)-1) my_palette <-
        ...
        hmap <- heatmap.2(t(OneSENSEmed), col = my_palette, breaks = breaks, margins = c(10, 20), Colv = FALSE, dendrogram = "row", 
            cexCol = 1, cexRow = 1, scale = "none", key = TRUE, trace = "none", density.info = c("none"), keysize = 1)
        ...
        if (OneDtSNEname == "input") {
            p1 <- plot_ly(z = hmapclu, y = rownames(hmapclu), x = testcol, colors = my_palette, type = "heatmap") %>% layout(title = paste(OneDtSNEname, 
                "Median heatplot", sep = " "))
            p1
        } else {
            p2 <- plot_ly(z = hmapclut, x = colnames(hmapclut), y = testcol, colors = my_palette, type = "heatmap") %>% layout(title = paste(OneDtSNEname, 
    ...
    OneSplot <- plot_ly(data.frame(Xx1DtSNEmat), x = Xx1DtSNEmat[, 1], y = Xx1DtSNEmat[, 2], type = "scatter", symbol = "circle-dot")
...
getParameters <- function(rawFCSdir) {
    ...
    markers <- paste("<", pd$name, ">:", pd$desc, sep = "")  ##STRING
...
getCoords <- function(LoaderPATH = LoaderPATH, FFdata = FFdata) {
    ...
    data1 <- apply(gcdata, 2, lgcl)  #apply logicle transformation to data
...
OneSmapperFreq2 <- function(LoaderPATH = "fcs", Bins = 250, FFdata) {
    ...
    for (factor in 2:(dim(keeptable)[2])) {
        ...
        fhmap <- heatmap.2(t(OneSENSEpp), col = my_palette, breaks = breaks, margins = c(10, 20), Colv = FALSE, dendrogram = "row", 
            cexCol = 1, cexRow = 1, scale = "none", key = TRUE, trace = "none", density.info = c("none"), keysize = 1)
