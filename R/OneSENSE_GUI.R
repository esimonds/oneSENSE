# @Yongkee
# Credits to @Chen Hao for his guidance and help with building the GUI
...
#'
#' @author Yong kee Tan
#' @return the GUI for oneSENSE analysis
#' @export
#' @import tcltk
#' @importFrom tcltk2 tk2checkbutton
#' @examples
#' onesense_GUI()
onesense_GUI <- function() {
  ...
  fcsFile <- tclVar("")
  #resDir <- tclVar("")
  #projectName <- tclVar ("OneSense")
  mergeMethod <- tclVar("ceil") #string
  fixedNum <-tclVar("10000")
  markers <- tclVar("") #replacement for names.csv?
  markers2 <- tclVar("")
  selectMarkers <- NULL
  selectMarkers2 <- NULL
  ret_var <- tclVar("") #return variable
  bins <- tclVar("250")
  frequency <- tclVar(0)
  ...
  reset_para_data <- function() {
    ...
    if (tclvalue(fcsFile) == "" && tclvalue(rawFCSdir) == "") {
      tkmessageBox(title = "OneSENSE: Catergorical Analysis of T-cell markers",
                   message = "Please input your \"rawFCSdirectory\" or \"fcsFile\".",
                   icon = "info", type = "ok")
    }
    if (tclvalue(fcsFile) != "") {
      fcsFiles <- strsplit(tclvalue(fcsFile), "}{", fixed = TRUE)[[1]] #fcsFiles is a LIST
    }
    fcsDir <- tclvalue(rawFCSdir)
    selectMarkers <<- getParameters_GUI(fcsFiles, fcsDir)

    if (length(selectMarkers) > 0) {
      tclvalue(markers) <- paste(selectMarkers$Marker[selectMarkers$input == "Y"], collapse = "}{")
    }

  }

  reset_para_data2 <- function() {
    ...
    fcsDir <- tclvalue(rawFCSdir)
    selectMarkers2 <- getParameters_GUI(fcsFiles, fcsDir)


    if (length(selectMarkers) > 0) {
      tclvalue(markers2) <- paste(selectMarkers2$Marker[selectMarkers2$input == "Y"], collapse = "}{")
    }
    #print(selectMarkers)
    #print(selectMarkers2)
    final <- cbind(selectMarkers, input2 = selectMarkers2[,2])

    #print(final)
    message("Parameters saved under ", dirname(tclvalue(rawFCSdir)))
    write.csv(final, paste(dirname(tclvalue(rawFCSdir)), "names.csv", sep=.Platform$file.sep), row.names = FALSE)
  }
  ...
  bins_help <- function() {
    tkmessageBox(title = "bins", message = "Allocate the number of bins to split tSNE data into",
                 icon = "info", type = "ok")
  }

  freq_help <- function() {
    tkmessageBox(title = "Frequency", message = "Allow for frequency selection, please head to R to select coordinates once prompted",
                 icon = "info", type = "ok")
  }
  ...
  reset <- function() {
    ...
    #tclvalue(projectName) = "Onesense"
    tclvalue(mergeMethod) = mergeMethods[3]
    tclvalue(fixedNum) = "5000"
    tclvalue(markers) = ""
    tclvalue(markers2) = ""
    tclvalue(bins) = "250"
    tclvalue(frequency) = 0
  }
  submit <- function() {
    has_error = FALSE
    if (tclvalue(markers) == "" || tclvalue(markers2) == "") {
      tkmessageBox(title = "OneSense",
                   message = "Please select the markers for your analysis in each catergory.",
                   icon = "info", type = "ok")
      has_error = TRUE
    }
  ...
  tkwm.title(tt, "OneSENSE: One-dimensional soli-expression by nonlinear stochastic embedding")
  ...
  ## markers1
  markers_label <- tklabel(tt, text = "Markers1 :")
  ...
  ## markers2
  markers2_label <- tklabel(tt, text = "Markers2 :")
  markers2_entry <- tkentry(tt, textvariable = markers2, width = box_length)
  markers2_button <- tkbutton(tt, text = " Select... ", width = bt_width,
                              command = reset_para_data2)
  markers2_hBut <- tkbutton(tt, image = image2, command = para_help)
  ...
  ## bins
  bins_label <- tklabel(tt, text = "Bins :")
  bins_hBut <-tkbutton(tt, image = image2, command = bins_help)

  bins_entry <- tkentry(tt, textvariable = bins, width = box_length)

  ## frequency
  freq_label <- tklabel(tt, text = "Frequency :")
  freq_hBut <- tkbutton(tt, image = image2, command = freq_help)
  freq_entry <-tk2checkbutton(tt, variable = frequency,tip = "Check this to perform frequency heatmap")
  ...
  tkgrid(markers2_label, markers2_hBut, markers2_entry, markers2_button,
         padx = cell_width)
  tkgrid.configure(markers2_label, markers2_entry, markers2_button,
                   sticky = "e")
  tkgrid.configure(markers2_hBut, sticky = "e")
  ...
  tkgrid(bins_label, bins_hBut, bins_entry, padx = cell_width)
  tkgrid.configure(bins_label, sticky = "e")
  tkgrid.configure(bins_hBut, sticky = "e")
  tkgrid.configure(bins_entry, sticky = "w")

  tkgrid(freq_label, freq_hBut, freq_entry, padx = cell_width)
  tkgrid.configure(freq_label, sticky = "e")
  tkgrid.configure(freq_hBut, sticky = "e")
  tkgrid.configure(freq_entry, sticky = "w")
  ...
  if (tclvalue(ret_var) != "OK") {
    ...
    inputs <- list() # "fcs", ceil, "names.csv", outputSuffix, dotSNE, DooneSENSE, bins
    inputs[["fcsdir"]] <- tclvalue(rawFCSdir)
    ...
    inputs[["bins"]] <- suppressWarnings(as.numeric(tclvalue(bins)))
    inputs[["frequency"]] <- as.logical(as.numeric(tclvalue(frequency)))
    print(inputs[["fcsdir"]])

    FCStSNE(LoaderPATH = inputs[["fcsdir"]],
            ceil =  inputs[["fixedNum"]],
            FNnames = "names.csv",
            OutputSuffix = "Out",
            DotSNE = TRUE,
            DoOneSENSE = TRUE,
    ...
    OneSmapperFlour(LoaderPATH =paste0(inputs[["fcsdir"]],"_","Out"),
                    Bins = inputs[["bins"]],
                    doCoords=TRUE,
                    doFreq=inputs[["frequency"]])
...
#' @return a dataframe containing selected markers per catergory
...
getParameters_GUI <- function(fcsFile, rawFCSdir) {
  ...
  markerChoice <- tclVar("")
  mm <- tktoplevel()
  tkwm.title(mm, "OneSENSE: Marker selection")
  ...
  input1[match(paras, markers)] <- "Y"
  #input2 <- rep("", length(markers_desc))
  #input2[match(paras2, markers_desc)] <- "Y"
  names <- data.frame(Marker=markers_desc, input = input1)
  #write.csv(names, paste(rawFCSdir, "names.csv", sep=.Platform$file.sep))
  return(names)
}
