...
oneSENSE_GUI <- function() {
  ui <- fluidPage(titlePanel("OneSENSE"), hr(), sidebarLayout(sidebarPanel(shinyDirButton("directory",
                                                                                          "Folder select", "Please select a folder"), tags$h4("FCS Folder Selected:"),
                                                                           verbatimTextOutput("directorypath"), actionButton("firstMarkers", "Display Category Markers"),
                                                                           actionButton("updatenamescsv", "Confirm Marker Selection"), numericInput("num",
                                                                                                                                                    "Ceiling Input", value = 500), numericInput("bins", "Bins", value = 250),
                                                                           actionButton("submit", "Submit"), actionButton("submit2", "Coordinate Selection"),
                                                                           br(), br(), actionButton("submit3", "Generate Frequency heatmap")), mainPanel(tabsetPanel(id = "Tabset1",
                                                                                                                                                                     tabPanel("Marker selection", value = "MkrSelect", flowLayout(checkboxGroupInput("markers1",
                                                                                                                                                                                                                                                     "Select 1st category markers", ""), checkboxGroupInput("markers2", "Select 2nd category markers",
                                                                                                                                                                                                                                                                                                            ""), checkboxGroupInput("markers3", "Select 3rd category markers (Optional)",
                                                                                                                                                                                                                                                                                                                                    ""))), tabPanel("Coordinate Selection", value = "CoordSelect", sidebarLayout(sidebarPanel(uiOutput("markerXdropdown"),
                                                                                                                                                                                                                                                                                                                                                                                                                              uiOutput("markerYdropdown"), tableOutput("table")), mainPanel(plotOutput("plot",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       click = "plot_click"), actionButton("writecsv", "Generate CSV"))))))))
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$updatenamescsv, {
      ...
      if (is.null(input$markers3)) {
        ...
        write.csv(final, paste(dirname(parseDirPath(volumes, input$directory)),
    ...
    observeEvent(input$submit, {
      FCStSNE(LoaderPATH = parseDirPath(volumes, input$directory), ceil = input$num,
              FNnames = "names.csv", OutputSuffix = "Out", DotSNE = TRUE, DoOneSENSE = TRUE,
              Bins = input$bins)
      OneSmapperFlour(LoaderPATH = paste(parseDirPath(volumes, input$directory),
                                         "_Out", sep = ""), Bins = input$bins, doCoords = FALSE, doFreq = FALSE)
    })
    observeEvent(input$submit2, {
      myFFdata <- OneSmapperFreq1(LoaderPATH = paste(parseDirPath(volumes, input$directory),
                                         "_Out", sep = ""))
      myCoordslist <- getCoords(LoaderPATH = paste(parseDirPath(volumes, input$directory),
                                   "_Out", sep = ""), FFdata = myFFdata)
      ...
      output$plot <- renderPlot({
        ...
        plot(ExpX, ExpY, pch = ".", xlab = input$markerX, ylab = input$markerY,
             cex = 0.1, ylim = c(-1, 4.5), xlim = c(-1, 4.5))
      }, width = 400, height = 400)
      output$table <- renderTable(values$df, rownames = TRUE, digits = 10)
      observeEvent(input$writecsv, {
        write.csv(values$df, paste(dirname(parseDirPath(volumes, input$directory)),
                                   "Coords.csv", sep = .Platform$file.sep))
      })
    })
    observeEvent(input$submit3, {
      FFdataOSMF2 <- OneSmapperFreq1(LoaderPATH = paste(parseDirPath(volumes, input$directory),
                                         "_Out", sep = ""))
      OneSmapperFreq2(LoaderPATH = paste(parseDirPath(volumes, input$directory),
                                         "_Out", sep = ""), Bins = input$bins, FFdataOSMF2)
    })
  }
