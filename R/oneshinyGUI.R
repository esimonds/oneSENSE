...
onesense_GUI <- function() {
    ...
    ui <- fluidPage(titlePanel("OneSENSE"), hr(), sidebarLayout(sidebarPanel(shinyDirButton("directory", "Folder select", 
        "Please select a folder"), tags$h4("FCS Folder Selected:"), verbatimTextOutput("directorypath"), actionButton("firstMarkers", 
        "Display Category Markers"), actionButton("updatenamescsv", "Confirm Marker Selection"), numericInput("num", 
        "Ceiling Input", value = 500), numericInput("bins", "Bins", value = 250), actionButton("submit", "Submit"), 
        actionButton("submit2", "Coordinate Selection"), br(), br(), actionButton("submit3", "Generate Frequency heatmap")), 
        mainPanel(tabsetPanel(id = "Tabset1", tabPanel("Marker selection", value = "MkrSelect", flowLayout(checkboxGroupInput("markers1", 
            "Select 1st category markers", ""), checkboxGroupInput("markers2", "Select 2nd category markers", 
            ""), checkboxGroupInput("markers3", "Select 3rd category markers (Optional)", ""))), tabPanel("Coordinate Selection", 
            value = "CoordSelect", sidebarLayout(sidebarPanel(uiOutput("markerXdropdown"), uiOutput("markerYdropdown"), 
                tableOutput("table")), mainPanel(plotOutput("plot", click = "plot_click"), actionButton("writecsv", 
                "Generate CSV"))))))))
    
    server <- function(input, output, session) {
        
        ## SELECT FCS RAW DIRECTORY ## if (.Platform$OS.type == 'windows') { volumes <<- c('Root' , 'C:/') } else if
        ## (.Platform$OS.type == 'unix') { volumes <<- c('Root' , '/root') } else { volumes <<-c('Root' , '/') }
        ...
        observeEvent(input$updatenamescsv, {
            ...
            if (is.null(input$markers3)) {
                ...
                write.csv(final, paste(dirname(parseDirPath(volumes, input$directory)), "names.csv", sep = .Platform$file.sep), 
                  row.names = FALSE)
            } else {
        ...
        observeEvent(input$submit, {
            FCStSNE(LoaderPATH = parseDirPath(volumes, input$directory), ceil = input$num, FNnames = "names.csv", 
                OutputSuffix = "Out", DotSNE = TRUE, DoOneSENSE = TRUE, Bins = input$bins)
            OneSmapperFlour(LoaderPATH = paste(parseDirPath(volumes, input$directory), "_Out", sep = ""), Bins = input$bins, 
        ...
        observeEvent(input$submit2, {
            ...
            output$plot <- renderPlot({
                ...
                plot(ExpX, ExpY, pch = ".", xlab = input$markerX, ylab = input$markerY, cex = 0.1, ylim = c(-1, 
                  4.5), xlim = c(-1, 4.5))
