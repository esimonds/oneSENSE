...
FCStSNE <- function(LoaderPATH = "fcs", ceil = 5000, FNnames = "names.csv", OutputSuffix = "Out", DotSNE = TRUE, DoOneSENSE = TRUE,
    Bins = 250) {
    ...
    for (FFs in 1:NumBC) {
        ...
        # in this case, a specified number of rows are taken randomly from the matrix without replacement Fixup column names
    ...
    if (DoOneSENSE) {
        ...
        if (dim(keeptable)[2] == 4) {
            ...
            scatterplot3d(x = Xx1DtSNEmat[, 1], y = Xx1DtSNEmat[, 2], z = Xx1DtSNEmat[, 3], pch = ".", xlab = paste("tSNE1 ", colnames(Xx1DtSNEmat)[1],
                sep = ""), ylab = paste("tSNE2 ", colnames(Xx1DtSNEmat)[2], sep = ""), zlab = paste("tSNE3 ", colnames(Xx1DtSNEmat)[3],
                sep = ""))
        } else {
            # loop from 2 to 4
            for (factor in 2:(dim(keeptable)[2])) {
            ...
            plot(Xx1DtSNEmat[, 1], Xx1DtSNEmat[, 2], pch = ".", xlab = paste("tSNE1 ", colnames(Xx1DtSNEmat)[1], sep = ""), ylab = paste("tSNE2 ",
