...
OneSmapperFlour <- function(LoaderPATH = "fcs_Out", Bins = 250, doCoords = FALSE,
                            doFreq = FALSE)
{
  ...
  for (FFs in 1:FNumBC) {
    ...
    # fs[[FFs]]@parameters$name <- colnames(FFa) Add file label
  ...
  for (factor in 2:(dim(keeptable)[2])) {
    # edited code
    keeprowbool <- sapply(keeptable[, factor], function(x) any(x ==
                                                                 "Y"))
    ...
    OneSENSEmed <- apply(dataX, 2, function(x) (tapply(x, tSNEbins,
                                                       FUN = median)))  #tapply(x, index, FUN)
    OneSENSEmed[is.na(OneSENSEmed)] <- 0
    # data2 <- as.data.frame(cbind(data1,tSNEbins)) OneSENSEmed <-
    # aggregate(data1 ~ tSNEbins,data =data1, FUN = median, na.rm = F)
    # OneSENSEmed <- data2 %>% group_by(tSNEbins) %>% summarise(median)
    # ddply(data2, 'tSNEbins', summarise, Med = medianmax(year) -
    # min(year), nteams = length(unique(team))) test <- with(dataX,
    # tapply())


    pdfFN <- paste(dirname(LoaderPATH), paste0(OneDtSNEname, "_Freq.pdf"),
                   sep = .Platform$file.sep)
    pdf(file = pdfFN, width = 14, height = 6)
    # my_palette <- colorRampPalette(c('blue','green','yellow'))(n=430)
    breaks = seq(0, 3, by = 0.005)

    # you can put whatever colors you like here: my_palette <-
    # colorRampPalette(c('black','orange','yellow','white'))(n=length(breaks)-1)
    # my_palette <-
    # colorRampPalette(c('black','blue','green','yellow'))(n=length(breaks)-1)
    # my_palette <-
    # colorRampPalette(c('blue','green','yellow'))(n=length(breaks)-1)
    # my_palette <-
    # colorRampPalette(c('black','blue','green','yellow','darkred'))(n=length(breaks)-1)
    my_palette <- colorRampPalette(c("blue", "white", "red"))(n = length(breaks) -
                                                                1)
    # my_palette <-
    # colorRampPalette(c('blue','lightblue','green','yellow','darkorange','darkred'))(n=length(breaks)-1)



    hmap <- heatmap.2(t(OneSENSEmed), col = my_palette, breaks = breaks,
                      margins = c(10, 20), Colv = FALSE, dendrogram = "row", cexCol = 1,
                      cexRow = 1, scale = "none", key = TRUE, trace = "none", density.info = c("none"),
                      keysize = 1)
    hmapclu = t(OneSENSEmed)[hmap$rowInd, hmap$colInd]
    hmapclut <- t(hmapclu)
    testcol <- seq(from = min(Xx1DtSNEmat), to = (max(Xx1DtSNEmat) -
                                                    (max(Xx1DtSNEmat)/Bins)), by = (max(Xx1DtSNEmat)/Bins))
    if (OneDtSNEname == "input") {
      p1 <- plot_ly(z = hmapclu, y = rownames(hmapclu), x = testcol,
                    colors = my_palette, type = "heatmap") %>% layout(title = paste(OneDtSNEname,
                                                                                    "Median heatplot", sep = " "))
      p1
    } else {
  ...
  suppressWarnings(OneSplot <<- plot_ly(data.frame(Xx1DtSNEmat), x = Xx1DtSNEmat[,
                                                                                 1], y = Xx1DtSNEmat[, 2], type = "scatter", symbol = "circle-dot"))
  OneSplot
  suppressWarnings(combined <- subplot(OneSplot, p2, p1, nrows = 2, shareY = TRUE,
                                       shareX = TRUE))
  combined

  export(combined, file = paste(dirname(LoaderPATH), "groupone.png",
                                sep = .Platform$file.sep))
...
getCoords <- function(LoaderPATH = LoaderPATH, FFdata = FFdata) {
  ...
  gckeeprowbool <- apply(gckeeptable[, c(2, 3)], 1, function(x) any(x ==
                                                                      "Y"))
  ...
  # Min <- numeric(length = length(keepnames)) Mid <<- numeric(length =
  # length(gckeepnames)) Max <- numeric(length = length(keepnames))
}
...
OneSmapperFreq2 <- function(LoaderPATH = "fcs", Bins = 250) {
  ...
  for (factor in 2:(dim(keeptable)[2])) {
    ...
    for (pname in colnames(dataX)) {
      ...
      overthresh <- function(group) {
        ...
        percpos <- (sum(group > Coords[which(Coords$X == pname),
                                       2])/length(group)) * 100  # calculates percentage of cells that are positive per bin
        # print(percpos)
        return(percpos)
      }
    ...
    fhmap <- heatmap.2(t(OneSENSEpp), col = my_palette, breaks = breaks,
                       margins = c(10, 20), Colv = FALSE, dendrogram = "row", cexCol = 1,
                       cexRow = 1, scale = "none", key = TRUE, trace = "none", density.info = c("none"),
                       keysize = 1)
    fhmapclu = t(OneSENSEpp)[fhmap$rowInd, fhmap$colInd]
    fhmapclut <- t(fhmapclu)
    ftestcol <- seq(from = min(Xx1DtSNEmat), to = (max(Xx1DtSNEmat) -
                                                     (max(Xx1DtSNEmat)/Bins)), by = (max(Xx1DtSNEmat)/Bins))
    if (OneDtSNEname == "input") {
      suppressWarnings(d1 <- plot_ly(z = fhmapclu, y = rownames(fhmapclu),
                                     x = ftestcol, colors = my_palette, type = "heatmap") %>%
                         layout(title = paste(OneDtSNEname, "Frequency heatplot",
                                              sep = " ")))
      d1
    } else {
      suppressWarnings(d2 <- plot_ly(z = fhmapclut, x = colnames(fhmapclut),
                                     y = ftestcol, colors = my_palette, type = "heatmap") %>%
                         layout(title = paste(OneDtSNEname, "Frequency heatplot",
                                              sep = " ")))
      d2
    }
  ...
  export(fcombined, file = paste(dirname(LoaderPATH), "grouptwo.png",
                                 sep = .Platform$file.sep))
