...
FCStSNE <- function(LoaderPATH = "fcs", ceil = 5000, FNnames = "names.csv", OutputSuffix = "Out", DotSNE = TRUE, 
    DoOneSENSE = TRUE, Bins = 250) {
    ...
    for (FFs in 1:NumBC) {
        ...
        ## Downsample ##
        if (nrow(FFt) <= ceil) 
            FFa <- FFt else FFa <- FFt[sample(nrow(FFt), ceil, replace = FALSE), ]  #sample takes a sample of the specified size form the elements of x using either with or without replacement
        # in this case, a specified number of rows are taken randomly from the matrix without replacement Fixup
        # column names
        ...
        # rbind concatenates the rows to its respective rows. All 3 samples of 20000 rows together to make 60000
        # rows
    }
    ...
    # lgcl <- logicleTransform(w=0.1, t=500000, m=4.5, a=0) #hyperbolic sine transformation functions for
    # display of flow cytometry data.
    ...
    if (DoOneSENSE) {
        ...
        if (dim(keeptable)[2] == 4) {
            ...
            # plot(Xx1DtSNEmat[, 1], Xx1DtSNEmat[, 2], pch='.', xlab=paste('tSNE1 ',colnames(Xx1DtSNEmat)[1],sep=''),
            # ylab=paste('tSNE2 ',colnames(Xx1DtSNEmat)[2],sep=''), cex=1)
            scatterplot3d(x = Xx1DtSNEmat[, 1], y = Xx1DtSNEmat[, 2], z = Xx1DtSNEmat[, 3], pch = ".", xlab = paste("tSNE1 ", 
                colnames(Xx1DtSNEmat)[1], sep = ""), ylab = paste("tSNE2 ", colnames(Xx1DtSNEmat)[2], sep = ""), 
                zlab = paste("tSNE3 ", colnames(Xx1DtSNEmat)[3], sep = ""))
        } else {
            # dim(keeptable)[2] == 3 for loop from 2 to 4
            for (factor in 2:(dim(keeptable)[2])) {
                OneDtSNEname <- colnames(keeptable)[factor]
                keeprowbool <- sapply(keeptable[, factor], function(x) any(x == "Y"))
                keeprows <- subset(keeptable, keeprowbool)
            ...
            plot(Xx1DtSNEmat[, 1], Xx1DtSNEmat[, 2], pch = ".", xlab = paste("tSNE1 ", colnames(Xx1DtSNEmat)[1], 
                sep = ""), ylab = paste("tSNE2 ", colnames(Xx1DtSNEmat)[2], sep = ""), cex = 1)
        }
